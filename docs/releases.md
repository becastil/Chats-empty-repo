...
```bash
gh release download v0.3.47 \
  --repo becastil/Chats-empty-repo \
  --pattern "repo_scout-*" \
  --pattern "repo-scout-*.pyz" \
  --pattern SHA256SUMS
python3 -m pip install ./repo_scout-0.3.47-py3-none-any.whl
```
...
gh attestation verify repo-scout-0.3.47.pyz \
  --repo becastil/Chats-empty-repo
gh attestation verify repo_scout-0.3.47-py3-none-any.whl \
  --repo becastil/Chats-empty-repo
gh attestation verify repo_scout-0.3.47.tar.gz \
  --repo becastil/Chats-empty-repo
```
