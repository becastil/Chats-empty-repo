...
gh release download v0.2.8 \
  ...
  --pattern "repo_scout-*" \
  --pattern SHA256SUMS
python3 -m pip install ./repo_scout-0.2.8-py3-none-any.whl
```
...
gh attestation verify repo_scout-0.2.8-py3-none-any.whl \
  --repo becastil/Chats-empty-repo
gh attestation verify repo_scout-0.2.8.tar.gz \
  --repo becastil/Chats-empty-repo
```
