...
`pilot-requested`, `price-objection`, `existing-solution`, `not-a-fit`, and
`do-not-contact`. Use `drafted` only after a personalized message has been
saved for review through a permitted channel.
...
boundary, uses the source-preserving direct-outreach route, and gives a clear
opt-out promising no further contact. Record that calendar date in
`approved_on`.
...
Every nonblank ledger date and every CLI date option must use the exact,
...
This mode prints six unchecked criteria plus only the alias, permitted channel,
qualification counts, and the canonical source-preserving direct-outreach
route:

`https://repo-scout.becastil.chatgpt.site/?source=outreach#why-teams-buy`

It does not expose evidence URLs or draft text, does not edit the ledger, and
does not approve or send a message. The human reviewer must inspect the private
evidence and saved draft before using a guarded review decision. The sixth
...
Schema-6 content receipt over the normalized selected ledger row, private
draft, displayed campaign route, and six human checks. The Schema-6 content
receipt excludes the bundle's ledger-audit date, so unchanged material can be
approved or declined after the day the owner-only file was created. Both
generated decision commands carry that receipt and the reviewed notes path,
and use `YYYY-MM-DD` for both the new audit date and, on approval,
...
repeated, or negated price text, nearby post-price language saying the recipient
will not pay or no payment is required, and any competing dollar amount fail at
the same boundary. An approved message can therefore test willingness to pay
for the stated offer instead of an undisclosed or contradictory price.
...
containing spaces remains one shell argument. Leaving the literal unchanged
fails before the private ledger or draft notes are read. The command writes
the next complete bundle with `600` permissions and prints only an alias-free
...
When a human observes a reply, pilot request, price objection, existing
solution preference, rejection, or opt-out after the initial contact or
follow-up, stop the cadence without hand-editing the ledger:
...
`pilot-requested`, `price-objection`, `existing-solution`, `not-a-fit`, and
`do-not-contact`; a generic `replied` row may later move to one of the five
specific outcomes after a human observes it.
...
`pilot-requested`, `price-objection`, `existing-solution`, `not-a-fit`, or
`do-not-contact`, retaining the same alias and ledger path while requiring a
fresh observation date. Terminal outcome receipts emit no further action
command.
...
A terminal `existing-solution` records only human-observed substitute evidence
that the prospect prefers an existing tool or DIY workflow. It closes the
cadence, and its dedicated `existing_solution_objections` count increments
...
pilot-requested, price-objection, or existing-solution counts do not become
public demand or revenue evidence; only public pilot intake and cumulative
funnel labels do.
