...
test("server-renders the Repo Scout companion page", async () => {
  ...
  assert.match(html, /repo-scout-0\.3\.16\.pyz/i);
  assert.match(html, /python3 \/tmp\/repo-scout\.pyz --languages \./i);
  assert.match(html, /Download v/i);
  assert.match(
    html,
    /releases\/download\/v0\.3\.16\/repo-scout-0\.3\.16\.pyz/i,
  );
...
test("publishes accurate free CLI and paid pilot structured offers", async () => {
  ...
  assert.equal(software.softwareVersion, "0.3.16");
  assert.equal(
    software.downloadUrl,
    "https://github.com/becastil/Chats-empty-repo/releases/download/v0.3.16/repo-scout-0.3.16.pyz",
  );
