...
- Collect the first three public pilot requests, work only the schema-10 sales
  ...
  duplicate-key-safe joined-growth validation. After payment, verify the
  private delivery contract and work the ordered schema-9+ activation queue
  through payment-backed first-repository activation or terminal
  reconciliation before pursuing another sale. Compare readiness and purchase
  criteria by source.
