...
- A dependency-free production audit that reconciles canonical metadata, the
  ...
  link, the link's visitor-facing $299 price, and the browser's final canonical
  production destination after redirects.
...
- A shared controlled-error output boundary for the Sites candidate CLI that
  preserves printable diagnostics and JSON-serializes a complete
  presentation-unsafe `SiteCandidateError`, including archive or receipt paths
  and wrapped operating-system context, before stderr reaches an operator.
...
- Authority-bound Sites repository identity that normalizes standard Git,
  ...
  every non-default port. The pre-save verifier accepts the pending request's
  exact canonical identity, treats conventional `git@` as protocol-neutral,
  retains every other SSH or SCP username, and distinguishes those users'
...
- JSON-escaped rollout duplicate-key errors that keep decoded line, terminal,
  ...
  emitted.
- Single-line-safe unknown rollout key errors that preserve ordinary printable
  field names while JSON-escaping decoded presentation controls before any
  summary is emitted.
- Single-line-safe rollout evidence source and path errors that preserve
  ordinary printable names while JSON-escaping presentation controls and
  retaining exact paths as structured detailed JSON.
...
- One unambiguous `$299` disclosure required at the same decision-ready
  boundary, rejecting missing, repeated, negated, competing, or explicit
  nonpayment terms on either side of the price so an approved attempt tests
  willingness to pay for the stated offer.
...
- Schema-3 approval receipts that report the privacy-safe remaining-draft
  ...
  carry nonempty queues into a distinct post-contact-or-cancellation review
  handoff rather than leaving the next prospect stranded.
- Complete-review continuity after a content-bound approval, preserving
  ...
  approved message to be sent and recorded or explicitly canceled before the
  emitted shell-quoted `PRIVATE-REVIEW-PATH` command writes the next `600`
  bundle.
...
- Schema-6 pre-contact `review-declined` decisions that close an unsuitable
  draft or cancel an approved-but-unsent message without contact dates or
  attempted-prospect inflation.
...
- Schema-2 private contact receipts that report durable
  `approved_review_digest` identity independently from contact-time
  `content_revalidated` evidence, covering all four current and legacy
  combinations without exposing the internal compatibility marker.
...
- Single-line-safe bootstrap receipt duplicate- and unknown-key errors that
  preserve printable field names while JSON-escaping decoded presentation
  controls before policy verification output.
...
identity copied exactly from the pending request. The digest and both
repository options are required together. Save the candidate only after that
check proves the resolved repository is remote, separate from `origin`,
approval-matched including any non-default port, nonstandard SSH username, or
SCP relative-path mode, stable, and its exported `refs/heads/main` is the
receipt commit.
...
contact record succeeds, retain its schema-2 private receipt as the audit link
between that attempted send and its approval identity, then use the approval
